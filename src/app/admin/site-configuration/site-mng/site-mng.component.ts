...
export class SiteMngComponent implements OnInit {
  ...
  fetchlocationByLocationId(locationid: number) {
    ...
    this.service.GetLocationbyLocationId(locationid).pipe(withLoader(this.loaderService)).subscribe((response: any) => {
  ...
  fetchLocationByPincode(pincode: number) {
    this.service.GetLocationByPincode(pincode).pipe(withLoader(this.loaderService)).subscribe((response:any) => {
  ...
  getProjList() {
    this.service.GetProjectList().pipe(withLoader(this.loaderService),delay(500)).subscribe((response: any) => {
      const items = response?.result || [];
      console.log("project")
  ...
  submit() {
    if (this.form.invalid) {
      ...
      if (this.noLocationFound) {
        ...
        this.service.LocationCreate(_locationmodel).pipe(withLoader(this.loaderService)).subscribe({
          next: (res: any) => {
  ...
  createSite = (finalLocationId: string) => {
    ...
    if (state?.mode === 'edit' && state?.record) {
      ...
      this.service.SiteEdit(_siteconfigmodel).pipe(withLoader(this.loaderService)).subscribe({
        next: () => {
      ...
      this.service.SiteCreate(_siteconfigmodel).pipe(withLoader(this.loaderService)).subscribe({
        next: () => {
